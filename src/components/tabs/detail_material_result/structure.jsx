...
import { structureState } from "@/states/structure_selector";

export default function DetailResultStructure() {
                    ...
                    <table className="text-sm text-center" style={{ width: "100%" }}>
                        <thead>
                            <tr className="bg-orange-200">
                                <th rowSpan={3} colSpan={2} className="p-1 border border-slate-400">구조물</th>
                        ...
                        <tbody>
                            <tr>
                                ...
                                {
                                    structure.unitCounts.map((value, index) => (<td key={index} className="p-1 border border-slate-400">{value}</td>))
                                }
                            </tr>
                            <tr>
                                ...
                                {
                                    structure.manhole.map((value, index) => (<td key={index} className="p-1 border border-slate-400">{value}</td>))
                                }
                            </tr>
                            <tr>
                                <td className="p-1 border border-slate-400">핸드홀</td>
                                {
                                    structure.handhole.map((value, index) => (<td key={index} className="p-1 border border-slate-400">{value}</td>))
                                }
                            </tr>
                            <tr>
                                <td className="p-1 border border-slate-400">접속함</td>
                                {
                                    structure.connecthole.map((value, index) => (<td key={index} className="p-1 border border-slate-400">{value}</td>))
                                }
                            </tr>
                            <tr className="bg-slate-100">
                                <td className="p-1 border border-slate-400">계</td>
                                {
                                    structure.totalhole.map((value, index) => (<td key={index} className="p-1 border border-slate-400">{value}</td>))
                                }
                            </tr>
                        </tbody>
                    </table>
